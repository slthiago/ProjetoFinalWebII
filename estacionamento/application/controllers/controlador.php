...
class controlador extends CI_Controller {
    ...
    public function Index() {
        ...
        $cliente = $this->contatos_model->GetAll('nome');
        // Passa os contatos para o array que será enviado à home
        $dados['cliente'] = $this->contatos_model->Formatar($cliente);
...
//  echo __FUNCTION__;
    ...
    public function Salvar() {
        ...
        if ($validacao) {
            ...
            $status = $this->contatos_model->Inserir($clientes);
    ...
    public function Atualizar() {
        ...
        if ($validacao) {
            ...
            $status = $this->contatos_model->Atualizar($cliente['idcliente'], $cliente);
            // Checa o status da operação gravando a mensagem na seção
            if (!$status) {
                $dados['cliente'] = $this->contatos_model->GetById($cliente['idcliente']);
    ...
    public function Excluir() {
        ...
        $status = $this->contatos_model->Excluir($idcliente);
        // Checa o status da operação gravando a mensagem na seção
        if ($status) {
    ...
    private function Validar($operacao = 'insert') {
        ...
        switch ($operacao) {
            case 'insert':
                ...
                $rules['cpf'] = array('trim', 'required', 'is_unique[cliente.cpf]');
                $rules['Telefone'] = array('trim', 'required');
                break;
            case 'update':
                $rules['Nome'] = array('trim', 'required', 'min_length[3]');
