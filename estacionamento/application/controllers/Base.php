...
class Base extends CI_Controller {
	...
	public function Editar(){
		...
		$dados['cliente'] = $this->contatos_model->GetById($id);
	...
	public function Atualizar(){
		...
		if($validacao){
			...
			// Checa o status da operação gravando a mensagem na seção
			if(!$status){
				$dados['cliente'] = $this->contatos_model->GetById($contato['id']);
	...
	private function Validar($operacao = 'insert'){
		...
		switch($operacao){
			case 'insert':
				...
				$rules['cpf'] = array('trim', 'required', 'valid_cpf', 'is_unique[contatos.cpf]');
                                $rules['telefone'] = array('trim', 'required');
				break;
			case 'update':
				$rules['nome'] = array('trim', 'required', 'min_length[3]');
				$rules['cpf'] = array('trim', 'required', 'valid_cpf');
                                $rules['telefone'] = array('trim', 'required');
				break;
			default:
				$rules['nome'] = array('trim', 'required', 'min_length[3]');
